...
if [ -z ${SKIP_SIM+x} ]; then
  time PYTHONPATH=~/src/gh0st42/pons:$PYTHONPATH pypy3 sim_lc.py
else
...
PYTHONPATH=~/src/gh0st42/pons:$PYTHONPATH ~/src/gh0st42/pons/tools/ponsanim/ponsanim.py -o $TMP_FILE -e $LOG_FILE -s 600 -d 50 -H -x store -x bundles_rxtx -x app_rxtx
