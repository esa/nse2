...
CONTACTS_FILE=contacts.ccp
ACTIONS_FILE=actions.txt
VIZ_FILE=viz.json

# Set a default startup timeout
# This can be adjusted based on the expected startup time of the nodes
STARTUP_TIMEOUT=30
SESSION_NAME=simple
# Count the number of nodes in the compose file
...
# all example nodes should print "OK: " after installing iproute2
wait_for_nodes "OK:"
...
# start the actions
tmux send-keys -t $SESSION_NAME "nse2_actions $ACTIONS_FILE" C-m
...
tmux send-keys -t $SESSION_NAME "nse2_mgr $COMPOSE_FILE $CONTACTS_FILE" C-m
# split the window vertically
tmux split-window -t $SESSION_NAME:1 -v
# start the visualization
tmux send-keys -t $SESSION_NAME "nse2_netviz $VIZ_FILE" C-m

# create new window for node terminals
tmux new-window -t $SESSION_NAME:2 -n 'Node Terminals'

# split the window horizontally
tmux send-keys -t $SESSION_NAME "nse2_sh n1" C-m
tmux split-window -t $SESSION_NAME:2 -h
tmux send-keys -t $SESSION_NAME "nse2_sh n2" C-m
tmux split-window -t $SESSION_NAME:2 -h
tmux send-keys -t $SESSION_NAME "nse2_sh n3" C-m
