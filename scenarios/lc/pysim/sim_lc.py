...
DEFAULT_TTL = 3600 * 24 * 3  # 72h


random.seed(RANDOM_SEED)

netplan = pons.net.NetworkPlan.from_graphml("lc.graphml")

print(netplan.nodes())
print(netplan.connections())

contactplan = pons.CoreContactPlan.from_file("lc.contacts.ccp", netplan.mapping)
...
static = pons.routing.StaticRouter(
    capacity=CAPACITY,
    graph=netplan.full_graph,
    pick_random_next_hop=False,
...
r1_tm_hk_sender = pons.apps.PingApp(
...
u1_tm_hk_sender = pons.apps.PingApp(
    dst=netplan.mapping["u1cc"],
    interval=300,
    ttl=DEFAULT_TTL,
    size=1024,
    rnd_start=True,
)
b1_tm_hk_sender = pons.apps.PingApp(
    dst=netplan.mapping["b1cc"],
    interval=300,
    ttl=DEFAULT_TTL,
    size=1024,
    rnd_start=True,
...
netsim.install_app("lgw", isp_b_tm_hk_sender)
netsim.install_app("rcc2", tm_receiver)

netsim.install_app("rover1", r1_tm_hk_sender)
netsim.install_app("r1cc", tm_receiver)

netsim.install_app("user1", u1_tm_hk_sender)
netsim.install_app("u1cc", tm_receiver)

netsim.install_app("base1", b1_tm_hk_sender)
netsim.install_app("b1cc", tm_receiver)
